...
/* Compute indexes */
inline static unsigned index(unsigned x, unsigned y) { return device::config.width; }
inline static unsigned index(unsigned x, unsigned y, unsigned z) { return device::config.width; }

//// +=============================================+
//// +==+ Device-Specific kernels and functions +==+
//// +=============================================+
//
...
//    lattice.set(x_i, y_i, device::INITIAL_CONFIG);
//    lattice_t.set(x_i, y_i, device::INITIAL_CONFIG);
//
//    /*
//    ** TODO: TO REMOVE
//    ** Put a circle at the center of the simulation
//    */
//    unsigned rel_x = lattice.WIDTH / 2 - x_i;
//    unsigned rel_y = lattice.HEIGHT / 2 - y_i;
//    double r = sqrt(static_cast<float>(rel_x * rel_x + rel_y * rel_y));
//
//    if (r < min(lattice.WIDTH, lattice.HEIGHT) * 0.05) {
//        lattice.obstacle(x_i, y_i) = lattice_t.obstacle(x_i, y_i) = true;
...
//
///* Stream the fluid */
//__global__ void stream(GpuLattice lattice, GpuLattice lattice_t) {
...
//
//        if (x_t > 0 && y_t > 0 && x_t < lattice.WIDTH && y_t < lattice.HEIGHT) {
//            lattice_t.f(x_t, y_t, i) = lattice.f(x_i, y_i, i);
//        }
//
//        /* Handle boundaries */
//        if (y_i == lattice.HEIGHT - 1 || y_i == 0 || x_i == lattice.WIDTH - 1 || x_i == 0) {
//            lattice_t.set(x_i, y_i, device::INITIAL_CONFIG);
//        }
//    }
//}
...
//__global__ void collide(GpuLattice lattice) {
//    unsigned x_i = blockIdx.x * blockDim.x + threadIdx.x;
//    unsigned y_i = blockIdx.y * blockDim.y + threadIdx.y;
//
//    Real rho;
...
//__global__ void bounce(GpuLattice lattice_t) {
//    unsigned x_i = blockIdx.x * blockDim.x + threadIdx.x;
//    unsigned y_i = blockIdx.y * blockDim.y + threadIdx.y;
//
//    /* Sadly, lots of threads are going to diverge here */
//    if (!lattice_t.obstacle(x_i, y_i)) {
...
//    device_lattice.swap(device_lattice_t);
//}
//
...
//    device_lattice.init(w, h);
//    device_lattice_t.init(w, h);
//    host_lattice.init(w, h);
...
//GpuSolver::~GpuSolver() {
//    device_lattice.free();
//    device_lattice_t.free();
//    cudaDeviceReset();
//}
//
//Lattice *GpuSolver::get_lattice() {
//    /* Synchronize with device lattice */
//    device_lattice.sync(host_lattice);
//    return &host_lattice;
//}
