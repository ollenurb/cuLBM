...
#define BLOCK_DIM 10 // Thread blocks are BLOCK_DIMxBLOCK_DIM
...
__global__ void init_kernel(LatticeNode *lattice, LatticeNode *lattice_t) {
    ...
    lattice[index] = lattice_t[index] = device::INITIAL_CONFIG;
...
GpuSimulation::GpuSimulation(unsigned int w, unsigned int h) : Simulation(w, h) {
    ...
    for (int i = 0; i < Q; i++) {
        ...
        tmp_init_conf.f[i] = D2Q9::W[i] * (1 + (3 * e_dp_u) + (4.5 * (e_dp_u * e_dp_u)) - (1.5 * tmp_init_conf.u.mod_sqr()));
    }
